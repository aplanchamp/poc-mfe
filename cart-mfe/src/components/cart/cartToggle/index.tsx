...
import CartPanelWrapper from '../cartPanelWrapper';
import styles from './cartToggle.module.scss';

const CartToggle = () => {
  const [cartOpened, toggleCart] = useState(false);
  return (
    <>
      <button
        className={styles.cartToggleButton}
        onClick={() => {
          toggleCart(!cartOpened);
        }}>
        <svg
          className={styles.cartToggleBag}
      ...
      {cartOpened &&
        createPortal(
          <CartPanelWrapper closePanel={() => toggleCart(false)} />,
          document.body
        )}
    </>
  );
